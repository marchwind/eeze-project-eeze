...
public class SADeviceReturnTest {
	
	@Autowired
	CommonDao<SADeviceReturn> dao;
	@Autowired
	CommonDao<SADeviceRental> rentalDao;
	@Autowired
	...
	public void testList() {
		SADeviceReturn common = insertDeviceReturn();
		common = insertDeviceReturn();
		
		try {
			List<SADeviceReturn> commonList = dao.list(common);
	...
	public void testInfo() {
		...
		try {
			SADeviceReturn commonSelected = dao.info(common);
			assertTrue(commonSelected.getDEV_RTN_NO()+"=="+common.getDEV_RTN_NO(), commonSelected.getDEV_RTN_NO().equals(common.getDEV_RTN_NO()));
		} catch(Exception e) {
			e.printStackTrace();
			fail("error occur");			
		}
	...
	public void testUpdate() {
		SADeviceReturn common = insertDeviceReturn();
		common.setRTN_STTS_EXPL("00");
		common.setREG_DTT(new Date());	
...
//			assertTrue(common.getDEV_MGMT_NO()!=null);
	...
	private SADeviceReturn insertDeviceReturn() {
		SADeviceRental rental = insertDeviceRental();
		SADeviceReturn common = TestUtils.initDeviceRental(rental.getDEV_RNT_NO());
		
		try {
			dao.insert(common);
			assertTrue(common.getDEV_RTN_NO()!=null);
			return common;
		} catch(Exception e) {
			e.printStackTrace();
			fail("error occur");		
